/*
 * libcwx.h version 1.0
 * A compilation of things you might want for CWX dev.
 * Contians: 
 *  - Type definitions 
...
void tui_draw_byte(byte x, byte y, byte data, byte mask);
...
// Graphics
#define VRAM 0xD000
#define Write2RealScreen *((volatile __near byte *)0xD601)

// Heap
#define HEAP_START_ADDR (VRAM + 0x602)
#define HEAP_MAX_SIZE 0x800
#define HEAP_BLOCK_ALIGN 2

// File system
#define FS_START_ADDR (HEAP_START_ADDR + HEAP_MAX_SIZE)
#define FS_MAX_SIZE 0xC00
...
enum tui_colour {
	TUI_COLOUR_WHITE,
	TUI_COLOUR_BLACK,
	TUI_COLOUR_IMAGE = TUI_COLOUR_BLACK // Nothing
};
...
byte tui_get_pixel(byte x, byte y) {
	...
	if (Write2RealScreen) {
		...
		return (deref((y << 4) + (y << 3) + (x >> 3) + VRAM) & ty) ? 1 : 0;
	}
}
		...
		case TUI_COLOUR_BLACK:
			for (iy = 0; iy < height; iy++) {
				for (ix = 0; ix < bwidth; ix++) {
					...
					tui_draw_byte(x + (ix << 3) - ax, y - ay + iy, bitmap[mapindex], mask);
				}
			}
			return;
		default:
